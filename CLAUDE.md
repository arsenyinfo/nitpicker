...
Tool outputs are intentionally a bit self-describing: `read_file` includes file/range headers, `glob`/`grep` return explicit no-match messages, and truncation messages say when output is partial. `read_file` rejects `end_line < start_line` instead of clamping to one line. A runtime tool's `Error:`-prefixed text is classified as `ToolCallStatus::Error` and its truncated text stored as the trajectory `result` (`agent.rs::execute_tool_call`), so `reflect` counts failed git pipes and bad ranges as errors rather than successes.
