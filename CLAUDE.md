...
0. Diff review entrypoints call `detect_diff_context` once before fan-out. It resolves full HEAD,
   ...
   free-form `ask` runs do not receive this diff snapshot
...
3. `agree=true` from Critic → convergence, lane-local, ends that lane early — but agreement is literal: the opponent's latest verdict must be forwardable unchanged; any changed finding, field, uncertainty, caveat, or unresolved blocker requires `agree=false`. The tool definition repeats this invariant. Actor/Reviewer owns recall and discovery; Critic/Validator owns precision and normally evaluates only the submitted set, but an exact clean verdict on its first turn triggers one independent rubric-scoped pass over the changed surface. A critic reports all currently visible objections together instead of manufacturing serialized disagreement, and never repeats that independent pass on follow-up turns. A critic that agrees with a failed actor's `*Agent failed*` stub (or a failed critic, whose verdict defaults to `agree=false`) does not converge. Review candidates may use the optional Uncertainty field on the first actor turn; after validator feedback the actor must resolve and omit it or drop the finding, verify technical premises newly entering the verdict, and retain one smallest correction. Validators and both synthesis paths receive only the confirmed schema, which cannot represent unresolved uncertainty. An agreeing empty verdict is the exact `No findings. Great job! 🎉` sentinel; otherwise it contains schema blocks only, with no audit narrative
...
2. Parse per-agent JSONL tool traces, `aggregation.json`, and optional `attribution.json` into typed session records. Staged
   verdicts are required; outdated or malformed aggregation records reject that session instead of
   entering reflection with incomplete evidence. Attribution records the binary version, build
   revision when available, and a SHA-256 over rendered protocol prompts
...
- `attribution.json` records the nitpicker package version, build-time Git revision (with `-dirty` when applicable), and a SHA-256 over representative rendered protocol prompts. It is a sidecar rather than an `AggregationRecord` field so the reusable crate's public record literal remains source-compatible; old sessions simply render attribution as unavailable
