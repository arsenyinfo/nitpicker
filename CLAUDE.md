...
`GitTool` only allows a fixed allowlist of read-only subcommands. Commands are passed directly to `Command::new("git").args(tokens)` — no shell involved. The subcommand allowlist alone isn't a read-only guarantee, so `ensure_readonly_git` additionally validates the arguments: it rejects `--output`/`-o` on every subcommand (otherwise `diff`/`log`/`show` write command output to an arbitrary, traversable path) and restricts `branch`/`tag` to listing/query forms (no mutating flags like `-d`/`-D`/`-m`/`-a`, and no bare positional that would create a ref — a positional is allowed only alongside a query flag such as `--list`/`--contains`).
