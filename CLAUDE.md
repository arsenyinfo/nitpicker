...
crates/nitpicker-agent/  — published library crate `nitpicker-agent`
  ...
  telemetry.rs    span contract (`bounded`, attribute vocabulary) + test-only span capture layer
...
- `otel` (binary only): adds the OpenTelemetry SDK + OTLP/HTTP-protobuf exporter as a second
  subscriber layer (`src/telemetry.rs`). The library has no feature: it emits plain `tracing`
  spans unconditionally, which cost nothing when no layer is interested in them.
...
3. Successful outputs are collected, concatenated, and sent to the aggregator model in a single completion call, prefixed with the original task and the preset roster. The final finding schema requires a `Lens` field: workers use their exact preset name, and synthesis retains every evidence-contributing lens when it merges duplicates (lens attribution is distinct from forbidden reviewer/model attribution). Preset runs EXCLUDE failure stubs from the aggregator input (execution noise, not review evidence — logs/progress/degraded accounting carry them); the Ask path keeps its stubs. The gap itself is still stated: `prompts::coverage_gaps` appends an execution-coverage block after the roster naming every preset that lost jobs (`N of M jobs completed`, `(not reviewed)` at zero), since a rubric silently missing from the roster reads as reviewed clean. A fully covered run's prompt is byte-identical to before. If **every** job failed, `run_review` bails before the aggregator — synthesizing a verdict from nothing but failure notes would fabricate a confident review; in `pr --json` this surfaces as `status: "error"` and no comment is posted
4. The aggregator's response is printed to stdout only when complete: `ToolUse` and `MaxTokens` finish reasons fail synthesis, so an unexpected tool call or truncated fallback-route verdict cannot be presented as the final report. Debate meta-review enforces the same boundary. On a degraded preset run the harness prepends one line to the report (`output::with_partial_review_note`: `> ⚠️ Partial review: N of M review jobs failed. Not reviewed: <presets with zero surviving jobs>.`) so the PR comment and stdout never read as a clean bill of health while exit 3 says otherwise; the synthesis text itself is untouched and `aggregation.json` keeps the raw synthesis. Ask runs get no note. `ReviewOutcome.degraded` (some but not all jobs failed) makes the default-review/`ask` arms return `Exit::Degraded`, which `main.rs::finish` maps to exit 3 after printing — stdout is flushed there explicitly, and a flush failure is exit 1 (contract: 0 clean / 1 hard failure / 3 degraded; 2 is clap's usage-error code, deliberately unused; `pr` joins the same contract — its outcome propagates out of `run_pr` so the exit happens after the checkout-restore guards drop). `main` is synchronous: parse → `telemetry::init` → build the tokio runtime → `block_on(run)` → drop the runtime → `Telemetry::shutdown` → `finish`. Nothing calls `process::exit` anymore, so every guard drops and the exporter drains before the code is chosen
...
8. Output is governed by the `--json` flag (on `PrArgs`, scoped to `pr` only) which maps to the internal `OutputFormat` enum: `Text` keeps the legacy human stdout (report printed, then comment posted); `Json` posts the comment first (so its outcome is reflected in `comment_posted`), then writes one `PrReviewOutput` line to stdout via `output::emit_json` (which flushes, so the envelope lands whatever exit path follows). In JSON mode, `debate.rs` suppresses its cast-line/verdict `println!`s and the `termimad` verdict rendering (threaded via `DebateOptions.format`), and tracing is always routed to stderr — so stdout stays a single clean JSON object. The envelope carries the resolved ordered `presets` on success only — every error envelope omits the key, including failures after resolution (`PrReviewOutput::error` clears all optionals) — plus `degraded` (any job/turn failed; mirrored as exit code 3, emitted-then-exit so the JSON always lands) and `coverage` (per-preset attempted/succeeded counts in resolution order; entries with `succeeded > 0` identify evidence-producing angles, while exact counts retain partial coverage; envelope-only by design, never fed to the synthesis prompt). The envelope's `usage` block (`UsageReport`: `input_tokens`/`output_tokens`/`total_tokens`/`cached_input_tokens`/`cache_creation_input_tokens`/`subagents_spawned`) is aggregated from the run: `review::run_review` returns a `ReviewOutcome` and `debate::run_debate` a `DebateOutcome`, each folding every reviewer/debate-turn `AgentResult` (subagents + compaction already folded in) plus the aggregator/meta completion — whose usage was previously discarded — via `UsageReport::add`. It is **best-effort** metering: tokens are sourced only from successful `CompletionResponse`s, so a failed reviewer/subagent or a discarded retry contributes 0 (a lower bound on spend, not an exact meter). `usage` is `None` in the `status: "error"` envelope. The cache keys are additive — `SCHEMA_VERSION` stays at 1 since existing consumers that ignore unknown keys are unaffected — but note `input_tokens` itself changed meaning in 0.8.3 on the Anthropic path (it now includes cache reads), so a consumer computing cost must discount `cached_input_tokens` at the provider's cache-read rate rather than charging the whole prompt. Subprocess caveats (for callers): `gh` auth/rate-limit is shared across processes, `--repo` must be an existing dir, kill via process-group on timeout (blocking `git`/`gh` children don't get the signal otherwise), and set `log_trajectories=false` to avoid per-run session writes.

### Telemetry (`src/telemetry.rs`, `crates/nitpicker-agent/src/telemetry.rs`)

The `tracing` subscriber is a registry with two layers. The stderr formatter (every build) keeps
the pre-span log shape: its per-layer filter is `EnvFilter ∧ !is_span`, so it never sees spans
and never prefixes an event with a `chat{...}:` scope, whether or not an exporter is attached
(`RUST_LOG` precedence and the `-v`/`reflect` default levels are unchanged). The OTLP layer
(feature `otel`) sees **spans only** — never log events, since the `info!`/`warn!` lines carry
tool arguments and provider error bodies — and only spans whose target starts with `nitpicker`
(`exported_span`). Activation is env-only (`OtelEnv::activation`): `OTEL_EXPORTER_OTLP_[TRACES_]ENDPOINT`
non-empty and `OTEL_SDK_DISABLED` ≠ true; a protocol other than `http/protobuf` is decided **before**
any exporter is built and disables export with a warning (the HTTP exporter would otherwise fall
back silently). There is deliberately no `[telemetry]` config table: in `pr` mode the config comes
from the target repo's base branch, and an endpoint there would redirect the run's telemetry.
Resource: `service.name` = `OTEL_SERVICE_NAME` or `nitpicker`, `service.version` = package version,
other detectors default. `Telemetry::shutdown` runs after the runtime is dropped; its failure is
a stderr warning and never changes the exit code.

Span tree (tracing field names; `otel.name`/`otel.kind`/`otel.status_code` are the
tracing-opentelemetry conventions): `nitpicker.run {command, pr.number}` → `review`/`debate`
`{mode, task, presets, reviewers, session.id, degraded, usage}` → `review.job` / `debate.lane` →
`debate.turn {role, round, agree}` → `invoke_agent {gen_ai.agent.name/id, depth, model; turns,
tool_calls, subagents_spawned, usage}` → `chat {gen_ai.request/response.model, usage incl. cache
breakdowns, finish_reasons, nitpicker.queue_wait_ms}` → `llm.attempt {attempt, error.type =
failure class}` (its `gen_ai.request.model` is the route actually tried, which differs from the
`chat` span's logical model after an Alloy pick or a fallback failover); `execute_tool {gen_ai.tool.name, turn, tool.status}` (a subagent's `invoke_agent`
nests under its `spawn_subagent` span); `compact {reason, turn}`; `synthesis`; `reflect.analysis`;
`openrouter.probe`. The `chat` span starts **after** the semaphore permit so its duration is
provider latency; the wait is an attribute. Every `tokio::spawn` site creates its span outside the
spawn and `.instrument`s the task (spawned tasks don't inherit the current span); `join_all` waves
need nothing. Attribute policy: identifiers, counts and timings only, every string through
`telemetry::bounded` (128 chars); a model-invented tool name never names a span
(`gen_ai.tool.name = "unknown"`). Pinned by `exported_spans_never_carry_content` (sentinel in
prompt, project context, tool args/output/error, subagent task/result, invented tool name) and the
hierarchy tests in `agent.rs`/`llm.rs`, which use the test-only capture layer in the library's
`telemetry.rs`. That layer installs one **global** subscriber per test binary and routes spans by
thread-local: scoped per-test dispatchers race in `tracing-core`'s callsite-interest / max-level
caches (rebuilt on every register/unregister) and silently disable spans on other threads.
