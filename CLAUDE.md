...
- `TokenUsage` is built from rig's `Usage` through one `From` impl at every provider ingest point, which **normalizes `input_tokens`** to mean all prompt tokens including cache reads. Providers disagree natively — Anthropic reports cache reads outside `input_tokens`, OpenAI folds them into `prompt_tokens` — so the prompt size is derived from rig's `total_tokens` (which counts everything on both), falling back to the reported input when a gateway omits totals. `cached_input_tokens`/`cache_creation_input_tokens` are breakdowns of `input_tokens`, never second addends, and `total_tokens == input_tokens + output_tokens` holds for every consumer. Without this, a cache hit on an auto-caching provider reports a fraction of the real prompt (measured: 52 for a 3509-token prompt) and `ConversationUsageWindow::should_compact` never fires
