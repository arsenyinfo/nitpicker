...
`GitTool` only allows a fixed allowlist of read-only subcommands. Commands are passed directly to `Command::new("git").args(tokens)` — no shell involved. The subcommand allowlist alone isn't a read-only guarantee, so `ensure_readonly_git` additionally validates the arguments. It rejects `--output`/`-o` (and long-option abbreviations of `--output`) on every subcommand — otherwise `diff`/`log`/`show` write command output to an arbitrary, traversable path. For `branch`/`tag` it uses a **default-deny allowlist** (a flag-name denylist is bypassable because git prefix-abbreviates long options and accepts `=`-glued/`-x`-glued values, e.g. `--set-upstream-to=x`, `--unset-up`, `-uorigin`): a long flag is accepted only if it prefixes a known read-only flag (git then resolves it to that flag or errors on ambiguity — never to a mutation), short flags must match exactly (no glued value/cluster), and `--` is treated as the end-of-options marker. Read-only flags are split into **action** flags (`--list`/`--contains`/`--merged`/…) that authorize a trailing positional (pattern/commit) and **modifier** flags (`--color`/`--sort`/`--format`/…) that don't — so a bare positional or one gated only by a modifier (`branch --color <name>`, `branch -- <name>`) is rejected as a potential ref creation.
