...
- `CompletionResponse::text()` strips inline `<think>` chain-of-thought, which some providers (MiniMax/GLM/DeepSeek via OpenRouter) emit in the message content instead of a structured field. A block the provider never closes would otherwise swallow the answer along with the reasoning — a complete verdict then reads as an empty response and dies after four retries. So when an unclosed block leaves nothing behind, `recover_unterminated_think` subtracts the reasoning from the block body and keeps the remainder. The reasoning is only ever subtracted, never guessed: it comes from what rig surfaces separately as `AssistantContent::Reasoning`, whose `Text` blocks are concatenated with **no separator** (OpenRouter splits them mid-sentence, so a newline join stops the prefix matching and leaks the whole chain), while `Summary`/`Encrypted`/`Redacted` are excluded since they don't appear verbatim in the body. `scan_think_blocks` reports where the unclosed block *starts*, so several blocks or a stray closer can't misdirect recovery, and the split must fall on a word boundary — real responses run the answer straight on from the reasoning's final punctuation, so requiring whitespace there would reject the true case. Anything that doesn't match returns `""`: serving the body would pass chain-of-thought off as the answer *and* silence the empty-response retry that exists for a reply that never answered. A *closed* block that strips to nothing is genuinely answer-free and still returns `""`
...
- An empty response (no text, no tool calls) is reported with `empty_response_diagnosis`: `finish_reason` plus the output tokens spent. `MaxTokens` there means the budget was consumed before any content, which is a configuration problem that otherwise presents as a broken model
...
- Agent turns and compaction send **no** `max_tokens`, so the provider's own per-model limit applies; `[[reviewer]].max_tokens` sets one explicitly and `[aggregator].max_tokens` defaults to `DEFAULT_AGGREGATOR_MAX_TOKENS` (16384). A cap is a budget for reasoning *plus* the answer, so one set below what the model reasons through returns empty content rather than a shorter answer — the failure this default exists to prevent. `0` is rejected at config time
