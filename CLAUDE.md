...
and `pr --json` keep the full resolved list — they document resolution; coverage travels
separately as the envelope's `covered_presets`/`degraded` and the session record's `jobs`/`lanes`). A preset
...
3. Format each session into a compact markdown summary of agents, tool activity, per-lane/per-job outcomes, and final verdict — an error-flagged aggregation record renders its synthesis failure (status "synthesis failed"), never its empty text as a verdict
