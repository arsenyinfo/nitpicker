...
cargo run -- --repo . --no-debate --review-timeout-seconds 600
...
- Agent and debate turn loops default to 100 turns and can be overridden via config or CLI; parallel reviewer/preset jobs can additionally be bounded by the CLI-only `--review-timeout-seconds` wall-clock deadline
