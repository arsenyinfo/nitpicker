...
**0.6.3** — 2026-06-06
- Audit fixes. **Security:** the `git` tool's "read-only" allowlist gated only the subcommand, so a model could write anywhere via `git diff --output=<path>` (absolute or `../`-traversed) and create/delete/move refs via `git branch`/`git tag` — destructive against the user's real repo in `pr` in-place mode. `--output`/`-o` is now rejected on every subcommand, and the `branch`/`tag` porcelain (whose read/write modes are indistinguishable by arguments) is dropped from the allowlist in favour of the read-only `for-each-ref`/`show-ref` plumbing, which has no ref-creating mode and is safe for any argument shape. **PR restore:** restoring a **detached HEAD** was broken (`git switch -- <sha>` refuses a bare commit), silently stranding the user on `nitpicker/pr-N`; restore now uses `git switch --detach` for that case and runs from a `Drop` guard so it survives panics and early errors. **Retry classifier:** the HTTP-status matcher treated `code` as a substring, so transient errors like `error decoding response body … 404` were misclassified as permanent 4xx and lost their retries — keys are now matched as whole words, and a 5xx response with a 4xx nested in its body is correctly retried. **Debate:** a cycle-blocked or malformed terminal tool call (`submit_verdict`/`finish`) no longer ends the agent with an empty verdict — it retries instead.
