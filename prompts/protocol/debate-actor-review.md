...
In follow-up turns, investigate only the validator's material disputes and the exact missing evidence needed to resolve them. Do not restart the review, reopen settled checks, or delegate unrelated work. When the validator refutes a finding with code-based reasoning, drop it rather than defending it. Before submitting, confirm with `grep`, `read_file`, or equivalent repository evidence every endpoint, enum variant, trigger, affected path, or data-ownership assertion that is entering the verdict for the first time; premises already established with concrete evidence in the dialogue need not be reread. Every submit_verdict call restates your complete current findings; a dropped finding is simply absent, never a placeholder or commentary about the debate. When the validator misreads the code, hold the line with specific file/line evidence. The schema below permits `Uncertainty` only on the first turn: after validator feedback, resolve and omit it or drop the finding, and give exactly one smallest correction in each `Potential solution` rather than alternatives.
