...
Then check the reviewer's Coverage block against the review snapshot and this angle. The block is a reach check, not a document to edit: reject it (`agree=false`, naming the exact gaps) only when a relevant in-scope file or an obvious risk class for this angle was actually not inspected and that gap could plausibly change the finding set, or when a Not inspected entry plausibly matters. Wording, grouping, revision-id formatting, and files or classes you would merely have listed differently are not grounds for rejection. Verify coverage with targeted reads of the named files, not by re-reviewing the target; do not turn a coverage gap into a finding of your own.
...
Agreement is literal: set `agree=true` only when the opponent's latest verdict can be forwarded unchanged. Any change to the finding set, title, lens, priority, location, trigger, solution, or uncertainty requires `agree=false`, as does any unresolved blocker or caveat, or a coverage gap that meets the bar above. When the findings hold and no such gap exists, agree and forward the Coverage block as written rather than asking for it to be reworded. With `agree=false`, give a concise evidence-based critique that tells the reviewer exactly what must change. With `agree=true`, the verdict must be only confirmed finding blocks in this schema, followed by the reviewer's Coverage block unchanged, with no preamble, audit narrative, or uncertainty:
{{FINDING_SCHEMA}}

If you reject every submitted finding, set `agree=false` with the critique; the reviewer resubmits without them. Agree with an empty finding set only when the reviewer submitted none and the coverage holds: then the verdict is the line {{NO_FINDINGS}} followed by the reviewer's Coverage block unchanged.
