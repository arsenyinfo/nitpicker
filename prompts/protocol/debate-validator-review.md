...
If no findings survive and you found none, set `agree=true` and the verdict exactly to: {{NO_FINDINGS}}

{{INVESTIGATION_GUIDANCE}}

The findings under review come from one assigned angle — {{PRESET_NAME}}:
{{RUBRIC}}
Judge them against that rubric's evidence bar and hunt for missed issues within this angle only; other angles run as separate debates. Every finding in your verdict must set Lens to exactly `{{PRESET_NAME}}`.
