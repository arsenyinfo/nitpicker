...
Then check the reviewer's Coverage block against the review snapshot and this angle: every in-scope changed file relevant to the angle must appear under Inspected or, with a reason, under Not inspected. Reject the coverage (`agree=false`, naming the exact gaps) when a relevant file or an obvious risk class for this angle is missing, or when a Not inspected entry plausibly matters. Verify coverage with targeted reads of the named files, not by re-reviewing the target; do not turn a coverage gap into a finding of your own.
